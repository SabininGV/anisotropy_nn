...
data = np.empty([12]) # массив, в к-м будут храниться данные по всем моделям
...
for i in range(K,N+1):
    ...
    lines[n-1] = anisotropic_parameters(dn1,dt1,alpha1, dn2,dt2,alpha2)
    ...
    frac_params = np.array([dn1,dt1,e1_n,e1_t,aspect_ratio1,alpha1, dn2,dt2,e2_n,e2_t,aspect_ratio2,alpha2])
...
df.to_csv(str(cwd) + "\\frac_params_crack_density2_{}_{}.csv".format(K,N),index=None, header=['dn1','dt1','e1_n','e1_t','aspect_ratio1','alpha1','dn2','dt2','e2_n','e2_t','aspect_ratio2','alpha2'])
