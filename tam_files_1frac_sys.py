...
def anisotropic_parameters(dn,dt,alpha=5.0):
    return "lh= '0 (base)' ls= 65536 lac= 0.000000 las= 0.000000 laa= 0.000000 lab=0.000000 tf=0.000000 tg=0.000000 tf1={0:.6f} tf2=0.000000 tf3=0.000000 tdn1={1:.6f} tdn2=0.000000 tdn3=0.000000 tdt1={2:.6f} tdt2=0.000000 tdt3=0.000000 tfb=1 ".format(alpha,dn,dt)
...
data = np.empty([3]) # массив, в к-м будут храниться данные по всем моделям
...
for i in range(K,N+1):
    ...
    frac_params = np.array([dn,dt,alpha])
...
df.to_csv(str(cwd) + '\\frac_params.csv',index=None, header=['dn','dt','alpha'])
